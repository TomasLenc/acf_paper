...
# x_var <- 'snr'
# 
# p_fft_raw <- ggplot(filter(df_noise_effect_z, selected_for=='fft'), aes(log10(.data[[x_var]]), z_meter_fft_raw)) +
#     geom_point(aes(color=pat), alpha=alpha) + 
...
#     theme_cowplot()
# 
# p_acf_raw <- ggplot(filter(df_noise_effect_z, selected_for=='acf'), aes(log10(.data[[x_var]]), z_meter_acf_raw)) +
#     geom_point(aes(color=pat), alpha=alpha) + 
#     geom_hline(yintercept = 0, color='#5e5e5e') + 
#     scale_color_jco() + 
#     scale_x_reverse() +
...
#     theme_cowplot()
# 
# p_fft_subtr <- ggplot(filter(df_noise_effect_z, selected_for=='fft'), aes(log10(.data[[x_var]]), z_meter_fft_subtr)) +
#     geom_point(aes(color=pat), alpha=alpha) + 
#     geom_hline(yintercept = 0, color='#5e5e5e') + 
#     scale_color_aaas() + 
#     scale_x_reverse() +
...
#     theme_cowplot()
# 
# p_acf_subtr <- ggplot(filter(df_noise_effect_z, selected_for=='acf'), aes(log10(.data[[x_var]]), z_meter_acf_subtr)) +
#     geom_point(aes(color=pat), alpha=alpha) + 
#     geom_hline(yintercept = 0, color='#5e5e5e') + 
#     scale_color_jco() + 
#     scale_x_reverse() +
...
#     theme_cowplot()
# 
# plot_grid(p_fft_raw, p_acf_raw, p_fft_subtr, p_acf_subtr, nrow=2, ncol=2)
...
plot_dist_to_truth <- function(df, init_params=list('min_'=0, 'max_'=1, 'x50_'=1, 'slope_'=1.5)){
    # convert to factors
    ...
    # fit sigmoids
    m_acf <- fit_sigm(df[,var_name], df$r_acf_subtr, init_params=init_params)
    m_fft <- fit_sigm(df[,var_name], df$r_fft_subtr, init_params=init_params)
...
df <- read.csv(file.path(experiment_path, 'data', 
                         'maxlag-11lags_meterRel-0.8_meterUnrel-0.6_1.0_1.4', 
                         fname))

plot_dist_to_truth(df)
```
...
save_fig(file.path(data_path, fname), p, width=2, height=2)
p
