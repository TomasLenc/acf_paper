...
for i_rhythm=1:n_rhythms
    ...
    fpath = fullfile(par.eeg_path, 'syncrange'); 
    
    load(fullfile(fpath, ...
        sprintf('exp-syncrange_rhythm-%s_tapping.mat', ...
                rhythm_id)));
...
%     % normalize to eeg for plotting?
%     s = (s - min(s)) ./ (max(s)-min(s)) ;
%     s = s * (max(data(:)) - min(data(:))) + min(data(:)); 
    ...
    if get_acf_feat_from_x
        ...
        feat_acf(i_rhythm) = get_acf_features2(data, fs, ...
                                    lags_meter_rel, lags_meter_unrel);    
...
fname = sprintf('exp-syncrange_tapping'); 
