...
% colors
cmap_name = '-YlGn'; 
colors = num2cell(brewermap(length(emph_levels) + length(emph_levels), cmap_name), 2); 
colors = colors(1:length(emph_levels), :); 
...
for i_cond=1:length(emph_levels)
    
    emph = emph_levels(i_cond); 
        
    cond_labels{i_cond} = sprintf('%g', emph); 

    fprintf('calculating %d/%d\n', i_cond, length(emph_levels))

    ir_par = []; 
    ir_par.type = 'square'; 
    ir_par.eventDur = 0.100; % 4 ms for click, 100ms for tone
    ir_par.rampon = 0.0;
    ir_par.rampoff = 0.0; 
    ir = get_ir(ir_par, fs); 
...
for i_cond=[1, 3]
    ...
    for i=1:length(f.Children)
        if strcmpi(f.Children(i).Type, 'legend')
           f.Children(i).Title.String = 'emph. magn'; 
