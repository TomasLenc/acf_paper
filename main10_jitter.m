...
jitters = logspace(log10(0.001), log10(0.200), 6); 
% ------------------------------------------------

n_cond = length(jitters); 
...
for i_cond=1:n_cond
    ...
    cond_labels{i_cond} = sprintf('%.2f', jit); 
...
for i_cond=1:length(cond_to_plot)
    ...
    ax.YLim = [min(-max(abs(reshape([feat.data], 1, []))), -0.1), ...
               max(max(abs(reshape([feat.data], 1, []))), 0.1), ...
               ];
