...
fname = list.files(path=data_path, 
           pattern='irType-square_apFitMethod-irasa_onlyHarm-.*_noiseEffectZscoreACFvsFFT.csv')
...
p_fft_high_z <- plot_subtr_effect(df, method='fft', pats=6)
p_acf_high_z <- plot_subtr_effect(df, method='acf', pats=3)
# pick a pattern with low negative true zscore
p_fft_low_z <- plot_subtr_effect(df, method='fft', pats=c(4))
...
# load simulated data
fname = list.files(path=data_path, 
           pattern='irType-square_apFitMethod-irasa_onlyHarm-.*_noiseEffectDistACFvsFFT.csv')
