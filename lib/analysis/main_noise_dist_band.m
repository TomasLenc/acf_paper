...
bands = linspace(f_pat / 5, f_pat / 2, 3); 
...
col_names = {
    'pat', 'snr', 'sample', 'bandwidth', 'r_raw', 'r_subtr', 'r_subtr_band', 'z_snr' ...
    };
...
for i_pat=1:size(par.all_pats, 1)
    ...
    parfor i_snr=1:length(par.snrs)
        ...
        % get acf with noise correction (subtract 1/f)
        [acf_subtr] = get_acf(x, par.fs, ...
               'rm_ap', true, ...
        ...
        for i_band=1:length(par.bandwidths)
            ...
            % (subtract 1/f and keep only frequencies near response
            % harmonics)
            [acf_subtr_band] = get_acf(x, par.fs, ...
                   'rm_ap', true, ...
                   'ap_fit_method', par.ap_fit_method, ...
                   'response_f0', par.response_f0, ...
            ...
            % get features
            feat_acf_subtr_band = get_acf_features(...
                                 acf_subtr_band, lags, ...
                                 par.lags_meter_rel, par.lags_meter_unrel); 

            % get measures of disctance from ground truth
            r_subtr_band = corr(feat_acf_orig.vals', feat_acf_subtr_band.vals'); 
